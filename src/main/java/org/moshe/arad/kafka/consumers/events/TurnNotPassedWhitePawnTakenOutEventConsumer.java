...
@Component
@Scope("prototype")
public class TurnNotPassedWhitePawnTakenOutEventConsumer extends SimpleEventsConsumer {

	@Autowired
	private MongoEventsStore mongoEventsStore;
	
	Logger logger = LoggerFactory.getLogger(TurnNotPassedWhitePawnTakenOutEventConsumer.class);
	
	public TurnNotPassedWhitePawnTakenOutEventConsumer() {
	}

	@Override
	public void consumerOperations(ConsumerRecord<String,String> record) {
    	try{
    		TurnNotPassedWhitePawnTakenOutEvent turnNotPassedWhitePawnTakenOutEvent = convertJsonBlobIntoEvent(record.value());    		
        	logger.info("Event recieved, try to put it in events store...");	                
        	mongoEventsStore.addTurnNotPassedWhitePawnTakenOutEvent(turnNotPassedWhitePawnTakenOutEvent);
	...
	private TurnNotPassedWhitePawnTakenOutEvent convertJsonBlobIntoEvent(String JsonBlob){
		ObjectMapper objectMapper = new ObjectMapper();
		try {
			return objectMapper.readValue(JsonBlob, TurnNotPassedWhitePawnTakenOutEvent.class);
		} catch (IOException e) {
