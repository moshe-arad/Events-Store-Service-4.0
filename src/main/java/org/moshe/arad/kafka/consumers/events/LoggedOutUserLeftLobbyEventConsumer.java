...
public class LoggedOutUserLeftLobbyEventConsumer extends SimpleEventsConsumer {
	...
	public void consumerOperations(ConsumerRecord<String,String> record) {
    	try{
    		LoggedOutUserLeftLobbyEvent loggedOutUserLeftLobbyEvent = convertJsonBlobIntoEvent(record.value());    		
    		logger.info("Game Room Closed Event record recieved, " + record.value());	             
        	logger.info("Event recieved, try to put it in events store...");	                
        	mongoEventsStore.addLoggedOutUserLeftLobbyEvent(loggedOutUserLeftLobbyEvent);
