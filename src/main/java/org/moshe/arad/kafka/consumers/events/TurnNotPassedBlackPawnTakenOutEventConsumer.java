...
public class TurnNotPassedBlackPawnTakenOutEventConsumer extends SimpleEventsConsumer {
	...
	public void consumerOperations(ConsumerRecord<String,String> record) {
    	try{
    		TurnNotPassedBlackPawnTakenOutEvent turnNotPassedBlackPawnTakenOutEvent = convertJsonBlobIntoEvent(record.value());    		
        	logger.info("Event recieved, try to put it in events store...");	                
        	mongoEventsStore.addTurnNotPassedBlackPawnTakenOutEvent(turnNotPassedBlackPawnTakenOutEvent);
	...
	private TurnNotPassedBlackPawnTakenOutEvent convertJsonBlobIntoEvent(String JsonBlob){
		ObjectMapper objectMapper = new ObjectMapper();
		try {
			return objectMapper.readValue(JsonBlob, TurnNotPassedBlackPawnTakenOutEvent.class);
		} catch (IOException e) {
