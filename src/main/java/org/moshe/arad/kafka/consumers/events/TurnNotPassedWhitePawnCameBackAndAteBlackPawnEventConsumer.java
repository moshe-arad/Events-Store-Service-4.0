...
@Component
@Scope("prototype")
public class TurnNotPassedWhitePawnCameBackAndAteBlackPawnEventConsumer extends SimpleEventsConsumer {

	@Autowired
	private MongoEventsStore mongoEventsStore;
	
	Logger logger = LoggerFactory.getLogger(TurnNotPassedWhitePawnCameBackAndAteBlackPawnEventConsumer.class);
	
	public TurnNotPassedWhitePawnCameBackAndAteBlackPawnEventConsumer() {
	}

	@Override
	public void consumerOperations(ConsumerRecord<String,String> record) {
    	try{
    		TurnNotPassedWhitePawnCameBackAndAteBlackPawnEvent turnNotPassedWhitePawnCameBackAndAteBlackPawnEvent = convertJsonBlobIntoEvent(record.value());    		
        	logger.info("Event recieved, try to put it in events store...");	                
        	mongoEventsStore.addTurnNotPassedWhitePawnCameBackAndAteBlackPawnEvent(turnNotPassedWhitePawnCameBackAndAteBlackPawnEvent);
	...
	private TurnNotPassedWhitePawnCameBackAndAteBlackPawnEvent convertJsonBlobIntoEvent(String JsonBlob){
		ObjectMapper objectMapper = new ObjectMapper();
		try {
			return objectMapper.readValue(JsonBlob, TurnNotPassedWhitePawnCameBackAndAteBlackPawnEvent.class);
		} catch (IOException e) {
