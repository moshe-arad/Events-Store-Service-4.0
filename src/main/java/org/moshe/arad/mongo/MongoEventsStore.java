...
public class MongoEventsStore {
	...
	public LinkedList<BackgammonEvent> getEventsOccuredFrom(Date fromDate){
		...
		while(it.hasNext()){
			result.push(this.convertTo(it.next()));
		}
				
		result.addFirst(new NewUserCreatedEvent(null, -1, -1, null, "NewUserCreatedEvent", null)); 
		result.addLast(new NewUserCreatedEvent(null, -2, -2, null, "NewUserCreatedEvent", null)); 
	...
	private BackgammonEvent convertTo(MongoEvent mongoEvent){
		if(mongoEvent.getClass().equals(org.moshe.arad.mongo.events.NewUserCreatedEvent.class)){
			...
			NewUserCreatedEvent newUserCreatedEvent = new NewUserCreatedEvent(newUserCreatedEventMongo.getUuid(), newUserCreatedEventMongo.getServiceId(), newUserCreatedEventMongo.getEventId(), newUserCreatedEventMongo.getArrived(), "NewUserCreatedEvent",newUserCreatedEventMongo.getBackgammonUser());
			...
			NewUserJoinedLobbyEvent newUserJoinedLobbyEvent = new NewUserJoinedLobbyEvent(newUserJoinedLobbyEventMongo.getUuid(), newUserJoinedLobbyEventMongo.getServiceId(), newUserJoinedLobbyEventMongo.getEventId(), newUserJoinedLobbyEventMongo.getArrived(), "NewUserJoinedLobbyEvent", newUserJoinedLobbyEventMongo.getBackgammonUser());
