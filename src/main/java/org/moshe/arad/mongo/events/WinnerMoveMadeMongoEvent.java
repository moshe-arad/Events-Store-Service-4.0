...
@Document(collection="WinnerMoveMadeEvents")
public class WinnerMoveMadeMongoEvent implements IMongoEvent {
	...
	public WinnerMoveMadeMongoEvent() {
		
	}

	public WinnerMoveMadeMongoEvent(String mongoEventId, UUID uuid, int serviceId, int eventId, Date arrived,
	...
	public String toString() {
		return "WinnerMoveMadeMongoEvent [mongoEventId=" + mongoEventId + ", uuid=" + uuid + ", serviceId=" + serviceId
				+ ", eventId=" + eventId + ", arrived=" + arrived + ", clazz=" + clazz + ", userName=" + userName
				+ ", gameRoomName=" + gameRoomName + ", from=" + from + ", to=" + to + ", backgammonBoardJson="
				+ backgammonBoardJson + ", firstDice=" + firstDice + ", secondDice=" + secondDice + ", isWhite="
				+ isWhite + "]";
	}

	public static WinnerMoveMadeMongoEvent convertIntoMongoEvent(WinnerMoveMadeEvent event) {
		WinnerMoveMadeMongoEvent winnerMoveMadeMongoEvent = new WinnerMoveMadeMongoEvent();
		
		winnerMoveMadeMongoEvent.setUuid(event.getUuid());
		winnerMoveMadeMongoEvent.setArrived(event.getArrived());
		winnerMoveMadeMongoEvent.setClazz(event.getClazz());
		winnerMoveMadeMongoEvent.setUserName(event.getUserName());
		winnerMoveMadeMongoEvent.setGameRoomName(event.getGameRoomName());
		winnerMoveMadeMongoEvent.setFrom(event.getFrom());
		winnerMoveMadeMongoEvent.setTo(event.getTo());
		winnerMoveMadeMongoEvent.setBackgammonBoardJson(event.getBackgammonBoardJson());
		winnerMoveMadeMongoEvent.setFirstDice(event.getFirstDice());
		winnerMoveMadeMongoEvent.setSecondDice(event.getSecondDice());
		winnerMoveMadeMongoEvent.setWhite(event.isWhite());
		
		return winnerMoveMadeMongoEvent;
	}
